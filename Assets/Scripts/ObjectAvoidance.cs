...
public class ObjectAvoidance : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        else if (other.tag != "NPCCollision" && other.tag != "Ground" && other.tag != "Plane" && other.gameObject.layer != LayerMask.NameToLayer("Curb"))
        {
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        else if(collision.collider.tag != "NPCCollision" && collision.collider.tag != "Ground" && collision.collider.tag != "Plane" && collision.gameObject.layer != LayerMask.NameToLayer("Curb"))
        {
            Debug.Log(collision.collider.name);
            if (transform.parent.GetComponent<Rigidbody>().velocity != Vector3.zero)
            {
